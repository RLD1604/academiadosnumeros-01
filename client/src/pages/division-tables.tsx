/**
 * Tabuadas de Divisão - Academia dos Números
...
export default function DivisionTables() {
  const [selectedTable, setSelectedTable] = useState<number>(2);
  const [answers, setAnswers] = useState<Map<number, Answer>>(new Map());
  const [shuffledOrder, setShuffledOrder] = useState<number[]>(Array.from({ length: 10 }, (_, i) => i + 1));
  ...
  const startGame = () => {
    // Shuffle the order of divisors
    const newOrder = Array.from({ length: 10 }, (_, i) => i + 1);
  ...
  const handleAnswerChange = (divisor: number, value: string) => {
    setAnswers(prev => {
      const newAnswers = new Map(prev);
      newAnswers.set(divisor, {
        value,
        isCorrect: null,
  ...
  const verifyAnswers = useCallback(() => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      if (answer && answer.value.trim()) {
        ...
        const isCorrect = !isNaN(userAnswer) && userAnswer === i;
    ...
    setIsVerified(true);
    setShowResults(true);
  }, [selectedTable, timer.elapsedTime, formatTime, answers]);
  ...
  const getAnswer = (divisor: number): Answer => {
    return answers.get(divisor) || {
      value: '',
      isCorrect: null,
      correctAnswer: divisor
    };
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl relative">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-amber-600 font-medium italic px-4">
            Pratique cada tabuada de divisão separadamente e teste seu conhecimento!
          </p>
        </div>
        
        {/* Table Selector */}
        <div className="academia-container mb-8">
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-700 text-center mb-4 sm:mb-6">🎯 Escolha sua Tabuada de Divisão</h3>
          <div className="grid grid-cols-5 sm:grid-cols-9 gap-2 sm:gap-3 lg:gap-4 max-w-6xl mx-auto">
        ...
        <div className="academia-container mb-8">
          <h2 className="academia-title text-center mb-6">
            Tabuada de Divisão do {selectedTable}
          </h2>
          
          {/* Layout responsivo com proporções automáticas */}
          <div className="w-full max-w-7xl mx-auto px-2 sm:px-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
              {/* Primeira coluna: 5 primeiros números embaralhados */}
              <div className="space-y-2 sm:space-y-3 lg:space-y-4">
                {shuffledOrder.slice(0, 5).map(divisor => {
                  const answer = getAnswer(divisor);
                  const dividend = selectedTable * divisor;
                  return (
                    <div key={divisor} className={`flex items-center justify-between p-2 sm:p-3 lg:p-4 rounded-lg border-2 transition-all duration-300 ${getCellBackground(answer)}`}>
                      <div className="flex items-center text-sm sm:text-lg lg:text-xl font-bold text-blue-700 min-w-0">
                        <span className="whitespace-nowrap">{dividend} ÷ {selectedTable} =</span>
                      </div>
                      <div className="flex items-center ml-2 flex-shrink-0">
                        <Input
                          type="number"
                          value={answer.value}
                          onChange={(e) => handleAnswerChange(divisor, e.target.value)}
                          ...
                          placeholder="?"
                          data-testid={`input-division-${divisor}`}
                        />
                        {getCellIcon(answer)}
              ...
              {/* Segunda coluna: 5 últimos números embaralhados */}
              <div className="space-y-2 sm:space-y-3 lg:space-y-4">
                {shuffledOrder.slice(5, 10).map(divisor => {
                  const answer = getAnswer(divisor);
                  const dividend = selectedTable * divisor;
                  return (
                    <div key={divisor} className={`flex items-center justify-between p-2 sm:p-3 lg:p-4 rounded-lg border-2 transition-all duration-300 ${getCellBackground(answer)}`}>
          ...
          <div className="academia-container mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
              <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6">
                <div className="text-3xl font-bold text-green-600 mb-2" data-testid="text-correct-count">
                  {finalResults.correct}
                </div>
                <div className="text-green-700 font-semibold">Acertos</div>
              </div>
              <div className="bg-red-50 border-2 border-red-200 rounded-xl p-6">
                <div className="text-3xl font-bold text-red-600 mb-2" data-testid="text-wrong-count">
                  {finalResults.wrong}
                </div>
                <div className="text-red-700 font-semibold">Erros</div>
              </div>
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6">
                <div className="text-3xl font-bold text-blue-600 mb-2" data-testid="text-time">
                  {finalResults.time}
                </div>
        ...
        {showResults && (
          <div className="academia-container mt-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {Array.from({ length: 10 }, (_, i) => i + 1).map(divisor => {
                const answer = getAnswer(divisor);
                const dividend = selectedTable * divisor;
                return (
                  <div key={divisor} className={`p-4 rounded-xl border-2 text-center ${
                    answer.isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                  }`}>
                    <div className="font-bold text-lg text-gray-700">
                      {dividend} ÷ {selectedTable}
                    </div>
