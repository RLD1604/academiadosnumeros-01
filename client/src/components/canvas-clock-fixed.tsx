...
export default function CanvasClockFixed({ clock, size = 400, isDraggable = false, onTimeChange }: CanvasClockProps) {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (isDragging && e.touches.length > 0) {
        e.preventDefault();
        updateTimeFromMouse(e.touches[0].clientX, e.touches[0].clientY);
      }
    };
  ...
  // Função auxiliar para detectar qual ponteiro está mais próximo
  const detectClosestHand = useCallback((x: number, y: number) => {
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const { closest, closestDist } = detectClosestHand(x, y);
  ...
  }, [isDraggable, detectClosestHand]);

  // Handler de touch start no canvas
